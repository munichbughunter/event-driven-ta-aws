...
```bash
playwright-js$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
playwright-js$ sam local start-api
playwright-js$ curl http://localhost:3000/
```
...
```bash
playwright-js$ sam logs -n HelloWorldFunction --stack-name playwright-js --tail
```
...
```bash
aws cloudformation delete-stack --stack-name playwright-js
```
